...
#   /srv/apps/thepenmarket/build/   the server/ directory (Dockerfile, src, templates, static, data, media)
...
docker run -d --name "$NAME" --restart unless-stopped --network web --env-file "$DIR/.env" \
  -e PORT=8140 -e HOST=0.0.0.0 -e APP_DIR=/app -e SITE_ORIGIN="https://$HOST" -v "$DIR/uploads:/app/uploads" "$NAME:latest" >/dev/null
